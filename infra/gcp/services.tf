...
resource "google_cloud_run_v2_job" "seeder" {
  ...
  template {
    template {
      service_account = google_service_account.seeder[each.key].email
      ...
      containers {
        ...
        dynamic "env" {
          for_each = var.mount_runtime_secrets ? toset(lookup(var.runtime_secret_names, "seeder", [])) : toset([])
