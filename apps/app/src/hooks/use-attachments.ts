...
export function useAttachments() {
  const getDownloadUrl = useCallback(async (attachmentId: string): Promise<string | null> => {
  ...
  const deleteAttachment = useCallback(async (attachmentId: string): Promise<void> => {
    ...
    const response = await apiClient.delete(`/v1/task-management/attachments/${attachmentId}`);
    if (response.error) {
