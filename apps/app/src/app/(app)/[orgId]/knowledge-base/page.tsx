...
export default async function KnowledgeBasePage() {
  ...
  const [policiesResult, contextResult, manualAnswersResult, kbDocumentsResult] = await Promise.all(
    [
  ...
  const policies = allPolicies.filter((p) => p.status === 'published' && !p.isArchived);
  ...
  return (
    <PageWithBreadcrumb breadcrumbs={[{ label: 'Overview', current: true }]} className="px-6">
