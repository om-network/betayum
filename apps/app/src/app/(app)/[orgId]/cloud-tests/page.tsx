...
export default async function CloudTestsPage({ params }: { params: Promise<{ orgId: string }> }) {
  ...
  const [providersResponse, findingsResponse] = await Promise.all([
    serverApi.get<{ data: Provider[]; count: number }>('/v1/cloud-security/providers'),
    serverApi.get<{ data: Finding[]; count: number }>('/v1/cloud-security/findings'),
  ]);
  ...
  const providers = Array.isArray(providersResponse.data?.data) ? providersResponse.data.data : [];

  const findings = Array.isArray(findingsResponse.data?.data) ? findingsResponse.data.data : [];
