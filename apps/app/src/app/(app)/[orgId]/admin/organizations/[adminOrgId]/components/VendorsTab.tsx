...
export function VendorsTab({ orgId }: { orgId: string }) {
  ...
  const handleFieldChange = async (vendorId: string, field: string, value: string | null) => {
    ...
    const res = await api.patch(`/v1/admin/organizations/${orgId}/vendors/${vendorId}`, {
      [field]: value,
    });
    if (!res.error) {
  ...
  const handleTriggerAssessment = async (vendorId: string) => {
    ...
    await api.post(`/v1/admin/organizations/${orgId}/vendors/${vendorId}/trigger-assessment`);
                    ...
                    <TableRow key={vendor.id}>
                      ...
                      <TableCell>
                        <Select
                          ...
                          onValueChange={(val) => {
                            if (val) void handleFieldChange(vendor.id, 'category', val);
                          }}
                          disabled={isUpdating}
                        >
                          <SelectTrigger size="sm">
                            <span className="text-sm">
                              {CATEGORY_LABELS[vendor.category ?? 'other'] ??
                                formatLabel(vendor.category ?? 'other')}
                            </span>
                          </SelectTrigger>
                      ...
                      <TableCell>
                        <Select
                          value={vendor.status}
                          onValueChange={(val) => {
                            if (val) void handleFieldChange(vendor.id, 'status', val);
                          }}
                          disabled={isUpdating}
                        >
                          <SelectTrigger size="sm">
                            <Badge variant={STATUS_VARIANT[vendor.status] ?? 'outline'}>
