...
export function TasksTab({ orgId }: { orgId: string }) {
  ...
  const handleFieldChange = async (taskId: string, field: string, value: string | null) => {
    ...
    const res = await api.patch(`/v1/admin/organizations/${orgId}/tasks/${taskId}`, {
      [field]: value,
    });
    if (!res.error) {
                    ...
                    <TableRow key={task.id}>
                      ...
                      <TableCell>
                        <Select
                          ...
                          onValueChange={(val) => {
                            if (val) void handleFieldChange(task.id, 'department', val);
                          }}
                          disabled={isUpdating}
                        >
                          <SelectTrigger size="sm">
                            <span className="text-sm">
                              {DEPARTMENT_LABELS[task.department ?? 'none'] ??
                                formatLabel(task.department ?? 'none')}
                            </span>
                          </SelectTrigger>
                      ...
                      <TableCell>
                        ...
                        >
                          <SelectTrigger size="sm">
                            <span className="text-sm">
                              {task.frequency ? formatLabel(task.frequency) : '--'}
                            </span>
                          </SelectTrigger>
                          <SelectContent alignItemWithTrigger={false}>
                      ...
                      <TableCell>
                        <Select
                          value={task.status}
                          onValueChange={(val) => {
                            if (val) void handleFieldChange(task.id, 'status', val);
                          }}
                          disabled={isUpdating}
                        >
                          <SelectTrigger size="sm">
