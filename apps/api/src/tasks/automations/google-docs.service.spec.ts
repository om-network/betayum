...
describe('GoogleDocsService', () => {
  ...
  describe('createDocument', () => {
    it('creates a document and returns documentId + url', async () => {
      ...
      const [updateUrl, updateOpts] = fetchSpy.mock.calls[1] as [
        string,
        RequestInit,
      ];
      expect(updateUrl).toBe(
        'https://docs.googleapis.com/v1/documents/doc_abc:batchUpdate',
      );
      expect(updateOpts.method).toBe('POST');
      const updateBody = JSON.parse(updateOpts.body as string) as {
        requests: unknown[];
      };
  ...
  describe('appendToDocument', () => {
    it('appends content and returns success', async () => {
      ...
      expect(url).toBe(
        'https://docs.googleapis.com/v1/documents/doc_xyz:batchUpdate',
      );
  ...
  describe('readDocument', () => {
    ...
    it('throws BadGatewayException on Google API error', async () => {
      fetchSpy.mockResolvedValueOnce({
        ...
        status: 403,
        json: async () => ({
          error: { message: 'Insufficient permission to read document' },
        }),
      });
