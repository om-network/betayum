...
export class AdminOrganizationsService {
  ...
  async createSupportContext(params: {
    orgId: string;
    actorUserId: string;
    targetUserId: string;
  }) {
    const { orgId, actorUserId, targetUserId } = params;
    if (!targetUserId) {
    ...
    if (member.user.id === actorUserId) {
      throw new BadRequestException(
        'Platform admins cannot start support context for themselves',
      );
    }

    return {
      organizationId: member.organization.id,
      organizationName: member.organization.name ?? member.organization.id,
  ...
  async stopSupportContext(params: { orgId: string; actorUserId: string }) {
