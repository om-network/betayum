  ...
  db: {
    ...
    $transaction: jest.fn((fn: any) =>
      fn({
        ...
        frameworkSyncOperation: {
          create: jest.fn().mockResolvedValue({ id: 'fso_rb' }),
          update: jest.fn(),
        },
...
describe('FrameworkRollbackService', () => {
  ...
  beforeEach(async () => {
    ...
    const mod = await Test.createTestingModule({
      providers: [FrameworkRollbackService],
    }).compile();
  ...
  it('400s when a task created by the sync has been completed since', async () => {
    (db.frameworkSyncOperation.findUnique as jest.Mock).mockResolvedValue({
      ...
      undoPayload: {
        ...
        tasks: { created: ['tsk_new'], archived: [], contentUpdated: [] },
        policies: {
          created: [],
          archived: [],
          contentUpdated: [],
    ...
    (db.task.findMany as jest.Mock).mockResolvedValue([
      { id: 'tsk_new', completedAt: new Date() },
    ]);
