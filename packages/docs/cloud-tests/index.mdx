---
title: "Cloud Tests"
description: "Continuously assess your cloud posture by connecting AWS, Azure, and GCP to Betayum."
---

## What Cloud Tests do

Cloud Tests connect your cloud accounts to Betayum and continuously evaluate them against security and compliance controls. Each finding is mapped to the frameworks you track (SOC 2, ISO 27001, CIS, PCI DSS, HIPAA), so cloud misconfigurations show up directly in your compliance posture.

All connections are **read-only by default**. Betayum only makes `GET` / `list` calls against each cloud provider unless you explicitly opt in to auto-remediation (AWS).
