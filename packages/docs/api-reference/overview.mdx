...
description: 'Use the Betayum API to automate SOC 2 evidence, policies, Trust Access, security questionnaires, tasks, and compliance workflows.'
---

The Betayum API lets engineering, security, and compliance teams connect their internal systems to the compliance workflows they run in Betayum.
...
Most organization automation uses the `X-API-Key` header. Create scoped API keys inside your Betayum organization, keep them server-side, and grant only the permissions needed for the workflow you are building.

```bash
curl --request GET \
  --url "https://api.betayum.com/v1/organization" \
  --header "X-API-Key: $COMP_AI_API_KEY"
```

The reference covers Betayum's product APIs across compliance operations, not just a single workflow. API-key backed endpoints are the integration contract for organization automation, while app session routes are kept out of the public reference.
...
The endpoint reference is generated from the NestJS OpenAPI specification used by the Betayum API. Endpoint titles, descriptions, examples, visibility rules, and Mintlify metadata are maintained in the API source so generated pages stay aligned with product behavior as routes change.
...
Use production API calls against `https://api.betayum.com` and avoid storing API keys in client-side code.
