...
<AccordionGroup>
  ...
  <Accordion title="apps/app/.env">
    ...
    CLERK_AUTHORIZED_PARTIES="https://app.yourdomain.com,https://portal.yourdomain.com"
...
For portal, no auth-specific public URL build arg is required.

<Note>
`docker-compose.yml` passes build args from shell environment variables (for example, `${NEXT_PUBLIC_APP_URL}`). Export these before running `docker compose build`, or set them inline.
</Note>
...
export NEXT_PUBLIC_PORTAL_URL="https://portal.yourdomain.com"
```
...
<CardGroup cols={2}>
  <Card title="HTTPS & Domains" icon="lock">
    Place a reverse proxy (nginx, Caddy, Traefik) in front. Update `NEXT_PUBLIC_APP_URL`, `NEXT_PUBLIC_PORTAL_URL`, and your Clerk issuer/authorized-party settings to your public HTTPS domains.
  </Card>
...
| Mismatched app URLs | Ensure `NEXT_PUBLIC_APP_URL` and `NEXT_PUBLIC_PORTAL_URL` match your public domains |
| Build args not exported | Export `NEXT_PUBLIC_APP_URL` and `NEXT_PUBLIC_PORTAL_URL` before build |
