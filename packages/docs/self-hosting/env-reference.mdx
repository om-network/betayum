...
| Name | Service(s) | Phase | Required | Notes |
|------|-----------|-------|----------|-------|
| `APP_GCP_ACCESS_KEY_ID` | app, portal | runtime | conditional | GCS interoperability access key |
| `APP_GCP_SECRET_ACCESS_KEY` | app, portal | runtime | conditional | GCS interoperability secret |
| `APP_GCP_REGION` | app, portal | runtime | optional | Defaults to `auto` for GCS |
| `APP_GCP_BUCKET_NAME` | app, portal | runtime | conditional | General file storage bucket |
| `APP_GCP_ORG_ASSETS_BUCKET` | app | runtime | conditional | Organization logos, compliance certs |
| `APP_GCP_QUESTIONNAIRE_UPLOAD_BUCKET` | app | runtime | conditional | Security questionnaire uploads |
| `APP_GCP_KNOWLEDGE_BASE_BUCKET` | app | runtime | conditional | Knowledge base documents |
| `APP_GCP_ENDPOINT` | app, portal | runtime | optional | Defaults to `https://storage.googleapis.com` |

<Note>
GCP Cloud Storage is the preferred object-storage backend. The app still accepts legacy `APP_AWS_*` variables as a fallback for existing S3-compatible deployments.
</Note>
