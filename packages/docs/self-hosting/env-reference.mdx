...
| `AUTH_PRIMARY_DOMAIN` | api | runtime | optional | Root production domain for API-owned auth cookies and trusted subdomains, e.g. `betayum.com` |
| `AUTH_STAGING_DOMAIN` | api | runtime | optional | Root staging domain for isolated staging cookies, e.g. `staging.betayum.com` |
| `AUTH_COOKIE_DOMAIN` | api | runtime | optional | Explicit cross-subdomain cookie domain. If omitted, derived from `BASE_URL` for `app`, `api`, and `portal` hostnames |
| `AUTH_TRUSTED_ROOT_DOMAINS` | api | runtime | optional | Comma-separated root domains whose exact host or subdomains can send trusted auth origins |
| `AUTH_TRUSTED_ORIGINS` | api | runtime | optional | Comma-separated exact origin allowlist. When set, it replaces generated localhost/app/api/portal origins |
