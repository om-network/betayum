...
const requiredPipelineSnippets = [
  ...
  'execute',
  '--wait',
  '--service-account=${_DB_JOB_SERVICE_ACCOUNT}',
  '--set-cloudsql-instances=${_CLOUD_SQL_INSTANCE}',
  '--set-secrets=DATABASE_URL=betayum-${_ENVIRONMENT}-database-url:latest',
  'curl --fail',
  '/v1/health',
  '/api/health',
...
const expectedStepDependencies = {
  ...
  'push-seeder': ['build-seeder'],
  'deploy-migrator-job': ['push-migrator'],
  'deploy-seeder-job': ['push-seeder'],
